...
/*---------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------*/
/**
*@brief    imprimiValores() inicia una operacion para imprimir las coordenadas en un cierto rango.
...
* 
*\details   i representa un indice entre el rango de x1 a x2.
*
*\details	y guarda el resultado de la funcion que corresponde al valor de la coordenada en el eje "y";
*
*@param     vM        Guarda el dato "m" de la funcion.
*@param     vB        Guarda el dato "b" de la funcion.
*@param     x1       Aloja el rango inferior o izquierdo. 
*@param     x2       Aloja el rango superior o derecho.
*@return   void
*
* @author Lucero A.M. (28/11/2020)
*
*/
void imprimirValores(int vM, int vB, float x1, float x2){
	float i=0,y;
	printf("\n\n\t|\tx\t|\ty\t|");
	for(i=x1; i<=x2;i++){
		y=(vM*i)+vB;
...
/**
*@brief    graficarValores() da paso a la graficacion de la funcion.
*\details  En esta funcion se utilizan las funciones que ya se encontraban estructuradas en el archivo.
*
*\details   gnuplot_setstyle() es para elegir el aspecto de la linea a generar.
*
*\details   gnuplot_set_xlabel() y gnuplot_set_ylabel() asignan un nombre a los ejes en las posiciones "x" y "y".
*
*\details   gnuplot_cmd() y gnuplot_cmd() generan los ejes "x" y "y".
*
*\details   gnuplot_cmd()  establece el rango del eje "x".
*
*\details   gnuplot_plot_slope(); crea el grafico de la forma y=mx+b.
*
*@param     m        Guarda el dato "m" de la funcion.
*@param     b        Guarda el dato "b" de la funcion.
*@param     x1       Aloja el rango inferior o izquierdo. 
*@param     x2       Aloja el rango superior o derecho.
*@return   void
*
...
*
*/
void graficarValores(int m, int b, float x1, float x2){
	
	gnuplot_ctrl * h1;
	h1 = gnuplot_init();
	
	gnuplot_setstyle(h1, "lines");
	gnuplot_set_xlabel(h1, "Eje X");
	gnuplot_set_ylabel(h1, "Eje Y");
	gnuplot_cmd(h1, "set xzeroaxis lt -1");
	gnuplot_cmd(h1, "set yzeroaxis lt -1");
	gnuplot_cmd(h1, "set xrange [%.1f:%.1f]",x1,x2);
	gnuplot_plot_slope(h1,m,b, "Funcion de la forma y=mx+b");
	//gnuplot_cmd(h1, "plot [%.1f:%.1f] y=%ix+%i",x1,x2,m,b);
	sleep(system("pause"));
	gnuplot_close(h1);
